...
navbarContent.addEventListener("hide.bs.collapse", function () {
    ...
    setTimeout(() => {
        ...
        navbar.style.backgroundColor = "var(--nav-background-dark)";
    }, 100);
...
addIntersectionObserverFor($("#skills .row .col"), (entries, observer) => {
    ...
    if (entries[0].isIntersecting) {
        if (window.matchMedia("(max-width: 767px)").matches) {
            // Mobile
            // rows.setAnimation("scroll_mobile 90s linear infinite");
            rows.forEach((row, i) => {
                row.style.animation = `scroll_mobile ${60 * (3 - i) / 2}s linear infinite`;
            });
        } else {
